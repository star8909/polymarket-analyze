"""iter20: 정치 카테고리만 정밀 calibration buckets (0.05 단위).

iter15 (카테고리별 robust) 발견:
- politics N=33 (작음), other N=45 (대중)

iter20: 정치만 더 큰 sample 모아서 정밀 분석.
정치 마켓이 polymarket 가장 큼 (대선/총선 등).
"""
...
def is_political(question: str) -> bool:
    ...
    pol_kws = ['election', 'president', 'vote', 'senate', 'governor', 'congress',
               'trump', 'biden', 'harris', 'putin', 'primary', 'mayor', 'parliament',
               'prime minister', 'chancellor', 'cabinet', 'nominee', 'speaker']
    for kw in pol_kws:
        if kw in q:
            return True
    return False
...
def main():
    ...
    df['is_pol'] = df['question'].apply(is_political)

    pol = df[df['is_pol']]
    print(f"  vol 50k+ + 정치: {len(pol)}")

    target_tokens = set(pol['yes_token'].tolist())
    token_meta = {r['yes_token']: {'end_ts': int(r['end_ts']), 'actual_yes': int(r['actual_yes'])} for _, r in pol.iterrows()}

    print(f"\n  Streaming CSV (정치 마켓만)...")
    ...
    while True:
        ...
        if chunks % 30 == 0:
            print(f"    chunks={chunks}, markets={len(market_trades)}")
    ...
    print(f"  정치 calibration sample: {len(cb_df)}")
    ...
    grouped = cb_df.groupby('bin', observed=True).agg(
        ...
        avg=('closing_belief', 'mean'),
        actual=('actual_yes', 'mean'),
    ).reset_index()
    grouped = grouped[grouped['n'] >= 5]

    print(f"  {'bin':25s} {'N':>5} {'belief':>8} {'actual':>8}  Edge   Signal")
    for _, row in grouped.iterrows():
        edge = row['actual'] - row['avg']
        marker = "🚀 LONG Yes" if edge > 0.05 else "🔥 SHORT (No)" if edge < -0.05 else ""
        print(f"  {str(row['bin']):25s} {row['n']:>5.0f} {row['avg']:>8.3f} {row['actual']:>8.3f}  {edge:+.3f}  {marker}")

    out = {"n_political": int(len(cb_df))}
    out_path = RESULTS_DIR / "iter20_category_precise.json"
